...
def plot_one_box(x, im,im_s, color=None, label=None, line_thickness=3,text1=''):
    # Plots one bounding box on image 'im' using OpenCV
    assert im_s.data.contiguous, 'Image not contiguous. Apply np.ascontiguousarray(im) to plot_on_box() input image.'
    tl = line_thickness or round(0.002 * (im_s.shape[0] + im_s.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    cv2.rectangle(im_s, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
    
    crop1 = im_s[int(x[1]):int(x[3]), int(x[0]):int(x[2])]
    # cv2.imwrite('test.jpg',crop1)
    ...
    # img = cv2.resize(img, (int(w*550/h), 550))
    # h, w = img.shape[:2]
    ...
    # text1 = pytesseract.image_to_string(img, config='-l eng --oem 1 --psm 7 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890 -c page_separator=')
